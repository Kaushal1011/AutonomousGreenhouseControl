...
MINIBATCH_SIZE = 5000  # How many steps (samples) to use for training
UPDATE_TARGET_EVERY = 1000  # Terminal states (end of episodes)
MODEL_NAME = 'RL_AGC'
...
class DQNAgentSpecial:
    def __init__(self,action_space,observation_space,env=None, model=None):
    ...
    def create_model(self):
        model = Sequential()
        model.add(Dense(16, input_shape=(1,self.observation_space)))
        model.add(Dense(32))
        model.add(Dense(16))
    ...
    def get_qs(self, state):
        return self.model.predict(np.array(state).reshape(1, self.observation_space))[0]